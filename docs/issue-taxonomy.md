...
| **Priority** | When applicable | `priority:critical`, `priority:high` | Urgency signal for `/kaizen-deep-dive` and `/make-a-dent` |

**Priority labels** — Use when urgency matters beyond the standard level/area scoring:
- `priority:critical` — Blocks humans, blocks multiple PRs, or 3+ incidents. `/kaizen-deep-dive` surfaces these first.
...
**Filing rule:** If you create an issue without `kaizen` + level + area, you're making it invisible to `/kaizen-deep-dive` and `/kaizen-gaps`.
...
- **Prioritization:** `/kaizen-deep-dive` can weight issues by incident count and recency
