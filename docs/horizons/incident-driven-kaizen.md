...
The kaizen system says "has incidents > theoretical" in `/kaizen-deep-dive` issue selection, but has no data structure to measure this. 98% of open kaizen issues have zero comments. When an agent encounters friction matching an existing issue, there's no enforcement or habit to record it. Incidents evaporate — the same issue gets re-discovered, re-discussed, and sometimes re-filed as a duplicate.

Without incident data:
- **Prioritization is opinion-based.** `/kaizen-deep-dive` can't distinguish "mentioned once as theoretical" from "happened 5 times this week."
...
| **L4** | Incident-driven scoring | `/kaizen-deep-dive` reads incident count/recency as a first-class scoring signal | `cli-kaizen` exposes incident metadata; deep-dive uses it | Visible from here |
...
- Document the incident comment format (convention, not enforcement):
  ...
  **Details:** What happened, what the agent was doing, how it was resolved
  ```
- Update `/kaizen-write-plan` Phase 3 awareness: when gathering incidents, count incident comments on the issue
...
- Problem: `/kaizen-deep-dive` says "has incidents > theoretical" but can't measure it.
