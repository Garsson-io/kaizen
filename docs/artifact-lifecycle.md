...
| **Test + coverage reports** | `npm run test:ci` on CI | `artifacts/` (gitignored, per-run) → uploaded to the run page via `actions/upload-artifact@v4` → uploaded to Codecov via `codecov/codecov-action@v5` | Human reviewer (download from run page), Codecov dashboard, coverage-regression gate (vitest `coverage.thresholds` fails CI on drop below baseline), `scripts/coverage-summary.mjs` (writes Markdown table to `GITHUB_STEP_SUMMARY`) | Codecov trend history, future kaizen-gaps runs identifying recurring uncovered modules | `artifacts/` local dir is ephemeral (gitignored, rebuilt every run). Upload to run page + Codecov makes the snapshot durable. |
