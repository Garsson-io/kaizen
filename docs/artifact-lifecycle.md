...
| **Hook-activation verdict** | auto-dent stream processor (per run, from the `system.init` event — #843) | `state.json` `run_history` as `hook_activation` (machine-readable), per-run log banner + controller console (loud) | Operator (banner), merge-gating (#1220, not yet wired), batch-tier degraded-run count (#1501, not yet wired) | kaizen-gaps (how often do hooks silently fail to load?) | Metric in `state.json` survives crash. Banner in per-run log is durable next to the run; controller console is live-only. A `plugins:[]` Claude run (kaizen hooks absent → no review/dirty/stop gate) is never silent. |
