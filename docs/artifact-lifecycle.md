...
| **Run transcript** | auto-dent (post-run, per PR) or `attach-transcript` CLI (manual sessions) — #1508 | Scrubbed, size-capped `run-transcript` attachment on the PR (`<!-- kaizen:run-transcript -->`); full JSONL on disk (`logs/auto-dent/run-N-*.log`) | **kaizen-reflect** (ground impediments in the transcript, not recollection — cite a specific moment), human reviewer | **kaizen-gaps** (mine transcripts across PRs for recurring frictions: repeated CLI fumbles, gate-reconciliation dances, context bloat), future L3 transcript-mining pass | Attachment permanent on PR; secrets scrubbed before upload (I19, fail-closed). Full transcript on disk is ephemeral. |
