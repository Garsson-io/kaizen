...
| `pr-quality-checks.ts` | `PreToolUse` on `Bash` | Warn on PR create, block merge when source changed without test changes | `pre-push` advisory plus CI required check | 2 | Same or stronger at merge boundary | CI is the durable enforcement layer |
| `pr-quality-checks.ts` | `PreToolUse` on `Bash` | Require `Verification` section in PR body and remind on merge | GitHub Action for PR body policy, post-merge notifier | 2 | Same merge gate, weaker local UX | Good CI policy candidate |
