...
3b. **Related-area simplification/DRY refactor pass before review** — Sweep the entire area related to the ticket for avoidable duplication, competing mechanisms, parallel schemas/standards, dead code, and excess surface area. Reuse, consolidate, simplify, or delete what is in scope. If a broader refactor is not warranted, record the evidence; do not leave this as optional cleanup.
3c. **Context delegation evidence before review** — Delegate context-heavy, parallelizable, or self-contained sub-work to subagents when the work is broad. Record what was delegated, or record the explicit reason delegation was not applicable for a narrow task.
4. **Review battery: invoke `/kaizen-review-pr`** — Discover all review dimensions via `npx tsx src/cli-dimensions.ts list`. Get the review briefing (dimensions, data needs, priority signals). Decide subagent grouping based on PR size and dimension priority signals. Spawn subagents for ALL dimensions. Validate coverage — all dimensions must have findings before proceeding. Filter: drop confidence < 75, classify MUST-FIX (≥80, blocks merge) and SHOULD-FIX (75-79, fix if easy or file follow-up — never silently skip). Also scan learned failure modes from `prompts/review-*.md` (FM-N patterns). See `/kaizen-review-pr` for the full workflow. **After classifying, call `store-review-batch` to persist findings before Phase 4** — see the mandatory storage step in `/kaizen-review-pr` Phase 3.
