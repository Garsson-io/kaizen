...
| `scripts/auto-dent-provider.ts` | **Runtime provider capability contract** (#1580/#1134) — single source of truth for which provider/billing can run each auto-dent phase. Dispatchers and provider-plan validation consume this file; descriptive matrices must align to it, not replace it. |
...
| `scripts/auto-dent-anomaly-incidents.ts` | **Auto-dent anomaly incident filing** (#725, epic #1677) — at batch finalize, detects harness-owned anomaly signals from existing `RunMetrics`/batch state (`empty_success`, hook rejection, lifecycle critical, >3 PRs, failed runs, cost/duration outliers), searches before filing, and creates/reuses labeled GitHub incident issues linked from `progress/batch-complete`. Fail-open; no worker-prose log parsing. |
...
| `scripts/auto-dent-rsi.ts` | **RSI improvement proposals** (#1158, epic #1677) — at batch finalize, converts reflection/degradation evidence plus prior/current `batch-outcome` metrics into an idempotent `rsi-improvement-proposals` attachment on the progress issue. Proposals are bounded: target prompt/skill/process surface, source signals, required behavioral proof (I22/Policy 10 for prompt/skill targets), baseline metrics, cross-run improvement verdict, and deterministic accept/watch/reject evaluation against a later `BatchOutcome`. CLI: `summary` reads the attachment or a JSON file; `evaluate` compares a saved proposal set to a later outcome. This is the safe propose/validate/archive arm; it does not auto-apply prompt patches. |
...
| `src/phase-marker.ts` | **One marker formatter/parser** for `AUTO_DENT_PHASE` marker lines — `formatPhaseMarkerLine(phase, fields)` and `parsePhaseMarkers(text)`. Round-trip-pinned so a producer can't format a line the parser won't read. The `store-plan`/`store-testplan` CLI emits a `PLAN` marker through here (#1502) so the auto-dent console confirms the I3/I8 plan payload via a structured signal, not a prose regex (I29). |
| `src/provider-contract.ts` | **Shared subscription-provider primitive** (#1580) — `AgentProvider`, `SubscriptionAgentProvider`, `parseSubscriptionAgentProvider()`, and `subscriptionAgentProvider()` are the single shape/factory for Claude/Codex subscription CLI agent providers. Do not hand-roll `{ provider, billing: 'subscription-cli' }` in provider-aware code; import this helper. |
| `src/codex-agent.ts` | **Canonical Codex provider helper module** (#1580/#1144) — builds `codex exec` argv and parses/normalizes Codex JSONL into the shared agent/auto-dent contracts. `scripts/auto-dent-codex.ts` is only a compatibility re-export, so new Codex provider logic belongs here. |
| `src/spawn-claude.ts` | **The ONE provider-aware agent spawn primitive** (#1231/#1580 DRY) — fresh process, no shared context; Claude stream-json and Codex JSONL parsing + cost + timeout in one place. Consumed by `review-battery.ts` and `independent-judge.ts`; neither reimplements the spawn loop. Compatibility exports keep the historical `SpawnClaudeFn` test seam. Codex review now fails as `codex_review_failed`; do not reintroduce the old `codex_review_unsupported` class except as historical data in old artifacts. |
...
| `scripts/review-round.ts` | **Focused authoritative review rounds** (#1736) — operator CLI for selected/full/grouped review dimensions without the `review-fix` repair loop. `run` always writes a JSON artifact, `store` fail-closes on provider failures/MISSING/missing dimensions before promoting to `review/rN/*` attachments + sentinel, and `run-and-store` requires `--store-only-if-pass`. See `docs/review-round-cli.md`. |
...
| `.githooks/pre-push` | Shell dispatcher — agent-env shortcut then `npx tsx src/hooks/pre-push.ts`; `core.hooksPath=.githooks` set by `postinstall` script |
