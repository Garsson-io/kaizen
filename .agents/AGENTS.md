...
| `scripts/auto-dent-rsi.ts` | **RSI improvement proposals** (#1158, epic #1677) — at batch finalize, converts reflection/degradation evidence plus `batch-outcome` metrics into an idempotent `rsi-improvement-proposals` attachment on the progress issue. Proposals are bounded: target prompt/skill/process surface, source signals, required behavioral proof (I22/Policy 10 for prompt/skill targets), baseline metrics, and deterministic accept/watch/reject evaluation against a later `BatchOutcome`. This is the safe propose/validate/archive arm; it does not auto-apply prompt patches. |
