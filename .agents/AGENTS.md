...
| `src/hooks/pre-push.ts` | **Git pre-push hook** (epic #1059) — mechanistic L3 gate: agent-env gate + pushed-ref merged-branch block (I7) + needs_review gate creation. Evaluates Git pre-push stdin remote branch targets, so deleted PR branch recreation is blocked even when `HEAD` is elsewhere (#1536). Replaces fragile Bash parsing path for push detection (#909, #1057). See `docs/git-hooks-design.md`. |
...
- **Detect merged-branch state before pushing.** Check the branch you are actually pushing. For ordinary `git push` this is usually the current branch; for explicit refspecs it is the remote target branch from `refs/heads/<branch>`. Run:
  ...
  ```
  If the most recent PR is `MERGED` and there's no newer `OPEN` PR on the pushed target branch, you must branch off before pushing. If an `OPEN` PR already exists on the pushed target branch, pushing to extend it is fine (new round bump is correct). Deleting a stale merged remote branch is allowed; recreating it with new commits is not.
