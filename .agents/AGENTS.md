...
| `scripts/auto-dent-anomaly-incidents.ts` | **Auto-dent anomaly incident filing** (#725, epic #1677) — at batch finalize, detects harness-owned anomaly signals from existing `RunMetrics`/batch state (`empty_success`, hook rejection, lifecycle critical/degraded, process incomplete, >3 PRs, failed runs, cost/duration outliers), searches before filing, and creates/reuses labeled GitHub incident issues linked from `progress/batch-complete`. Fail-open; no worker-prose log parsing. |
