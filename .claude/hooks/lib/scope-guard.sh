...
try {
  const fs = require('fs');
  const d = JSON.parse(fs.readFileSync('$user_settings', 'utf8'));
...
" 2>/dev/null)
  ...
  if [ "$is_user_level" = "yes" ]; then
    ...
    # Uses node (native to kaizen's toolchain) rather than python3.
    #
    # Attempt counter: if node fails and the setting persists, stop
    ...
    if [ "$attempts" -ge 3 ]; then
      ...
      echo "[kaizen] Manual fix: node -e \"const fs=require('fs'),p=require('os').homedir()+'/.claude/settings.json',d=JSON.parse(fs.readFileSync(p,'utf8'));delete d.enabledPlugins['kaizen@kaizen'];fs.writeFileSync(p,JSON.stringify(d,null,2))\"" >&2
      return 0
    fi
...
if (d.enabledPlugins) delete d.enabledPlugins['kaizen@kaizen'];
