#!/bin/bash
# Tests for kaizen-check-verification.sh hook
# Run: bash .claude/kaizen/hooks/tests/test-kaizen-check-verification.sh
#
...
# SUT: kaizen-check-verification.sh

set -u

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
HOOKS_DIR="$(dirname "$SCRIPT_DIR")"
HOOK="$HOOKS_DIR/kaizen-check-verification.sh"
