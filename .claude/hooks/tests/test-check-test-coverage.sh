#!/bin/bash
# Tests for kaizen-check-test-coverage.sh hook
# Run: bash .claude/kaizen/hooks/tests/test-kaizen-check-test-coverage.sh
#
...
# SUT: kaizen-check-test-coverage.sh

set -u

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
HOOKS_DIR="$(dirname "$SCRIPT_DIR")"
HOOK="$HOOKS_DIR/kaizen-check-test-coverage.sh"
...
setup_gh_git_mocks ".claude/kaizen/hooks/kaizen-check-test-coverage.sh" "src/index.ts
src/unrelated.ts"
