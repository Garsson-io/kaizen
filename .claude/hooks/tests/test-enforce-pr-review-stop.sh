#!/bin/bash
# Tests for kaizen-enforce-pr-review-stop.sh — Stop hook that blocks Claude from
...
HOOK="$(dirname "$0")/../kaizen-enforce-pr-review-stop.sh"
setup_test_env
...
# SUT: kaizen-enforce-pr-review-stop.sh with empty STATE_DIR
...
# SUT: kaizen-enforce-pr-review-stop.sh via state-utils.sh staleness check
...
# SUT: kaizen-enforce-pr-review-stop.sh with mixed states
OUTPUT=$(run_stop_hook "false")
if is_blocked "$OUTPUT"; then
