#!/bin/bash
# Tests for kaizen-enforce-pr-review.sh — Level 3 PR review gate (Issue #46)
source "$(dirname "$0")/test-helpers.sh"

HOOK="$(dirname "$0")/../kaizen-enforce-pr-review.sh"
setup_test_env
...
# SUT: kaizen-enforce-pr-review.sh with empty STATE_DIR
...
# SUT: kaizen-enforce-pr-review.sh edge case handling
...
# SUT: kaizen-enforce-pr-review.sh with mixed state files on same branch
OUTPUT=$(run_gate "npm test")
if is_denied "$OUTPUT"; then
