...
# kaizen-verify-before-stop.sh needs TypeScript changes to be meaningful
# Test with no changes — should exit 0
run_single_hook "$HOOKS_DIR/kaizen-verify-before-stop.sh" "$STOP_INPUT" 10 "$(printf 'PATH=%s' "$MOCK_DIR:$PATH")"
...
# kaizen-check-cleanup-on-stop.sh should always exit 0
run_single_hook "$HOOKS_DIR/kaizen-check-cleanup-on-stop.sh" "$STOP_INPUT" 10
if [ "$HOOK_EXIT" -eq 0 ]; then
