...
echo ""
echo "=== kaizen-pr-reflect-clear.sh clears merge-triggered gate ==="

# The gate is still set from previous test — clear it via KAIZEN_NO_ACTION declaration
# (kaizen-pr-reflect-clear.sh triggers on KAIZEN_IMPEDIMENTS or KAIZEN_NO_ACTION, not gh issue create)
CLEAR_HOOK="$(dirname "$0")/../kaizen-pr-reflect-clear.sh"
CLEAR_INPUT=$(jq -n '{
  "tool_name": "Bash",
