...
7. **Create PR** — `gh pr create` with `Fixes $ISSUES_REPO#N` in body (cross-repo prefix required for auto-close). Add `status:has-pr` label to kaizen issue. Add PR link as comment. **Write the PR description as a narrative using the Story Spine arc** (adapted from Pixar's storytelling structure for technical context):
   - **Once upon a time...** — What was the world like before? What system existed, what was it doing?
   - **Every day...** — What routine was happening? What metrics said things were fine?
   - **One day...** — What incident or discovery revealed the problem? Be concrete — name the PR, the issue, the specific failure.
   - **Because of that...** — What did you build? Show real output, not just describe it. Include actual findings, error messages, or before/after data.
   - **Until finally...** — What's the proof it works? Empirical evidence, not claims. Link to artifacts (test results, follow-up PRs, validation data).
   - **And ever since...** — How does the world look now? What's different for the next person?
   Then follow with: Architecture (diagram or data flow), Design decisions (table: decision / why / tradeoff), What's in this PR (file table), Validation evidence (checked boxes with specifics), Known limitations (honest, with next steps).
   A reviewer should understand the PR's value, impact, and technical choices **without reading the diff**. The diff is the proof — the description is the argument.
