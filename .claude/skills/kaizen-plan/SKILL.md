...
This skill takes a spec document (from `/kaizen-prd` or equivalent) and breaks the implementation into independent, sequenced PRs — each with its own GitHub issue. The output is a dependency graph on the epic issue and a set of sub-issues ready for dev cases.
...
**When to use:**
- After `/kaizen-prd` has produced and merged a spec document
- When a large initiative needs implementation planning
- When you look at a spec and think "this is too big for one PR"

**Prerequisite:** A spec document exists (in `docs/` or linked from an epic issue). If not, run `/kaizen-prd` first.
...
Add the full breakdown to the epic issue (the one created by `/kaizen-prd`). Format:
...
- **`/kaizen-prd`** — Use first to create the spec document that this skill decomposes into PRs.
- **`/cases`** — Each sub-issue becomes a dev case when work begins.
- **`/kaizen-reflect`** — After each PR merge, kaizen reflections may spawn new issues.
