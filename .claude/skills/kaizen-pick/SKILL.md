...
**Role:** The selector. Chooses WHICH issue to work on next from the kaizen backlog. Avoids collisions with other agents, balances momentum against diversity. Feeds into `/kaizen-evaluate` — never skips straight to implementation.
...
```bash
gh pr list --repo "$HOST_REPO" --state open --json number,title,headRefName,labels
```
...
- Issues that are just a title with no body may need `/kaizen-prd` first
...
3. **First step** — would this go straight to `/kaizen-evaluate` → `/kaizen-implement`, or does it need `/kaizen-prd` first?
4. **Estimated scope** — small (< 1 PR), medium (1-2 PRs), large (needs `/kaizen-plan`)
...
1. **#N: Title** — [why: momentum from recent X work + unblocks Y]
   First step: /kaizen-evaluate → /kaizen-implement | Scope: small

2. **#N: Title** — [why: security cluster neglected, has 3 past incidents]
   First step: /kaizen-evaluate | Scope: medium
...
- Offer to create one: "Want me to file a kaizen issue for this and proceed to `/kaizen-evaluate`?"
- If the user agrees, create the issue with `gh issue create` including a clear problem statement, then flow to `/kaizen-evaluate`

### Step 6: Flow to accept-case

When the user selects an issue:
- Invoke `/kaizen-evaluate` to evaluate it before implementation
...
```
/kaizen-pick  →  /kaizen-evaluate  →  /kaizen-implement  →  /kaizen
