...
export const VERDICT_BINDING_INVENTORY: VerdictBindingInventory = {
  ...
  terminalActions: [
    ...
    },
    {
      ...
      enforcingConsumer: 'verifyIssuesClosed()/autoCloseKaizenIssues() require merged PR state; merge itself is gated by quality verdicts',
      failureModeBlocked: 'Issue closure only follows a merged PR, and auto-merge is denied when the quality verdicts are red.',
      sourceEvidence: [
        { file: 'scripts/auto-dent-github.ts', tokens: ['verifyIssuesClosed', "data.state !== 'MERGED'", 'gh issue close'] },
        { file: 'src/hooks/pr-kaizen-clear.ts', tokens: ['autoCloseKaizenIssues', "prState !== 'MERGED'", 'reconcileClosedIssueStatusLabels'] },
