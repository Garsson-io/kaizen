...
3. For every behavior row or bullet marked deferred, extract tracking issue references like `#123` or `Garsson-io/kaizen#123`.
...
- `MISSING`: A deferred behavior has no tracking issue.
- `MISSING`: A deferred behavior references only closed or missing tracking issues.
- `DONE`: A deferred behavior references at least one open tracking issue.
- `DONE`: No behaviors are marked deferred.
- `PARTIAL`: More than 30% of behavior rows are deferred. This is a warning, not a failure by itself, because the PR may still have open tracking issues for each deferral.
