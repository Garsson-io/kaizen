...
describe('auto-dent provider capability inventory', () => {
  ...
  it('rejects runtime capabilities that the descriptive matrix does not accept (#1580)', () => {
    const driftedRuntime = RUNTIME_PROVIDER_CAPABILITIES.map((cap) =>
      cap.provider === 'codex' && cap.phase === 'reflection' && cap.billingMode === 'subscription-cli'
    ...
    const driftedDescriptive = PROVIDER_CAPABILITIES.map((cap) =>
      cap.id === 'codex-structured-exec'
        ? { ...cap, phaseFit: { ...cap.phaseFit, reflection: 'avoid' as const } }
        : cap,
    );

    expect(validateProviderCapabilityRuntimeAlignment(driftedDescriptive, driftedRuntime)).toEqual(
      expect.arrayContaining([
        expect.stringContaining('runtime codex/reflection/subscription-cli'),
      ]),
    );
  });
